...
void Map::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
    ...
    for (int i = 0; i < base.get_kol(); i++) {target.draw(*base.get_obj(i));}
    target.draw(camera, states);
    target.draw(conture, states);
}

Map::Map(Character _player): 
    back(sf::Vector2f(sett->scale*sett->W, sett->scale*sett->H)),
    conture(sf::LineStrip, _player.ray_kol.x+2), camera(sett->scale*_player.size), kol(_player.ray_kol.x+2), part(sett->scale)
{
    back.setOutlineThickness(3);
    back.setOutlineColor(sf::Color::Black);
    camera.setFillColor(sf::Color::Black);
    camera.move(sett->scale*sf::Vector2f(_player.pos.x, _player.pos.y));
    for (int i = 0; i < kol; i++)
    {
        conture[i].position =sett->scale*_player.conture[i].position;
